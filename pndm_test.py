...
class People_Background(pl.LightningModule):
    ...
    def forward(self,laten:torch.FloatTensor=None,timesteps:torch.Tensor=None,
                people_feature:torch.FloatTensor=None,back_feature:Optional[torch.FloatTensor]=None,
                pose_img:torch.FloatTensor=None): 
                down_block_res_samples, mid_block_res_sample=self.controlnet_pose(
                        ...
                        return_dict=False,back_hidden_states=back_feature)
    ...
    def sample(self,people_img:torch.FloatTensor,part_img:torch.FloatTensor,
            ...
            for t in tqdm(self.val_scheduler.timesteps):
                ...
                latens_=self.val_scheduler.step(noise_pred,t,latens_).prev_sample
            return latens_
    ...
    def get_people_condition(self,people_img,part_img):
        ...
        people_local_feature=self.people_global_fusion(part_laten,people_global_feature)
        people_local_feature=self.people_local_fusion(people_local_feature)

        return people_local_feature

    def get_back_feature(self,background_img):
        back_feature=self.clip(background_img)[0].detach()
    ...
    def configure_optimizers(self):
        ...
        params =[i  for i in (list(self.people_proj.parameters())
                            +list(self.people_proj_part.parameters())
                            +list(self.people_global_fusion.parameters())
                            +list(self.people_local_fusion.parameters())
                            +list(self.unet.parameters())
                            +list(self.back_proj.parameters())
