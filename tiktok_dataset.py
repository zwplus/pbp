...
class diffusion_dataset(Dataset):
    def __init__(self,data_pairs_txt_list,if_train=True) -> None:
        ...
        # self.ref_transform_local = transforms.Compose([ # follow CLIP transform
        #     transforms.ToTensor(),
        #     transforms.RandomResizedCrop(
        #         (224, 224),
        #         scale=(1.0, 1.0), ratio=(1., 1.),
    ...
    def __getitem__(self, index):
        try:
            ...
            for i in os.listdir(local):
                t=Image.open(os.path.join(local,i))
                ref_local_img.append(self.transformer_clip(t).pixel_values[0])
